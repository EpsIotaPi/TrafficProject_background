...
class BiLstmCRF:
    ...
    def __init__(self, hp, is_training):
        ...
        self.total_key_depth = hp.total_key_depth
        self.total_value_depth = hp.total_value_depth
        ...
        attention = self.mul_head_att_layer(attention_nums=self.blocks, inputs=inputs_embedding,
                                            num_head=self.num_heads, rate=self.rate)
        states = self.bilstm_layer(attention, self.lstm_dim, "bilstm_layer", self.length)
    ...
    def mul_head_att_layer(self, attention_nums, inputs, num_head=8, rate=0):
        old = inputs
        new_attention = inputs
        for _ in range(attention_nums):
            new_attention = multihead_attention(old, old, num_heads=num_head, rate=rate)
            old = new_attention
